...
let encodeOption = (opt: option('a), toString: ('a) => string): Js.Json.t => {
  switch (opt) {
    | Some(value) => Json.Encode.string(toString(value))
...
let encodeState = (s: state): Js.Json.t => {
  Json.Encode.object_([
    ("qty", encodeOption(s.qty, string_of_int)),
    ("size", encodeOption(s.size, Shirt.Size.toString)),
    ("sleeve", encodeOption(s.sleeve, Shirt.Sleeve.toString)),
    ("color", encodeOption(s.color, Shirt.Color.toString)),
    ("pattern", encodeOption(s.pattern, Shirt.Pattern.toString)),
...
let decodeOption = (field: string, json: Js.Json.t,
  fromString: (string)=> option('a)): option('a) => {
  
  let s = Json.Decode.field(field, Json.Decode.string,json);
  if (s == "----") {
    None
  } else {
    fromString(s)
...
let decodeState = (json: Js.Json.t): state => {
  {
    qty: decodeOption("qty", json, toInt),
    size: decodeOption("size", json, Shirt.Size.fromString),
    sleeve: decodeOption("sleeve", json, Shirt.Sleeve.fromString),
    color: decodeOption("color", json, Shirt.Color.fromString),
    pattern: decodeOption("pattern", json, Shirt.Pattern.fromString),
...
let make = () => {
  ...
  let initialState = getStoredState();
