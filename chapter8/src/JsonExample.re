...
let decodedStr =
  switch (Json.parse({js|"two worlds"|js})) {
  | Some(jsonStr) => D.string(jsonStr)
  | None => ""
  };
Js.log(decodedStr); // "two worlds"
...
let objStats = {js|{"title": "color", "choices": ["White", "Blue", "Red", "Green", "Brown"], "totals":[118, 114, 73, 67, 28]}|js};
