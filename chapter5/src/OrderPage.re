let commaSplit = (s: string): array('string) => {
  ...
  |> Belt.Array.map(_, Belt.Option.getWithDefault(_, ""));
};
...
let nShirts =
  commaSplit(str) |> Belt.Array.map(_, toOrder) |> calculateTotalShirts;
Js.log(nShirts); // 17
...
let setInnerHTML = (id: string, htmlString: string): option(unit) => {
  Doc.getElementById(id, D.document)
  |> Belt.Option.map(_, Elem.setInnerHTML(_, htmlString));
};

let setInnerText = (id: string, textString: string): option(unit) => {
  Doc.getElementById(id, D.document)
